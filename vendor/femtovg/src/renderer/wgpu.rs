...
impl<'a> From<&'a OwnedBindingResource> for wgpu::BindingResource<'a> {
    fn from(owned: &'a OwnedBindingResource) -> Self {
        match owned {
            ...
            OwnedBindingResource::ExternalTexture(texture_view) => {
                Self::ExternalTexture(texture_view)
            }
        }
    }
}
    ...
    fn render(
        ...
        self.pipeline_cache
            ...
            .retain(|_, cached_pipeline| {
                // Slint flushes its clear pass separately from its content pass.
                // Keep pipelines across those passes and cached-layer-only frames.
                cached_pipeline.unused_flushes = cached_pipeline.unused_flushes.saturating_add(1);
                cached_pipeline.unused_flushes <= 128
            });
...
fn concave_fill(
    ...
    if command
        .drawables
        .iter()
        .any(|drawable| drawable.fill_verts.is_some())
    {
        pipeline_and_bindgroup_mapper.update_renderpass(
            render_pass_builder,
            None,
    ...
    if command
        .drawables
        .iter()
        .any(|drawable| drawable.stroke_verts.is_some())
    {
        for drawable in &command.drawables {
            // draw fringes
            pipeline_and_bindgroup_mapper.update_renderpass(
...
impl BindGroupState {
    fn materialize(
        ...
        let (main_texture_view, main_sampler) =
            RenderPassBuilder::create_binding_resource_and_sampler(
                device,
                images,
                self.image.as_ref(),
                empty_texture,
            );
        let (glyph_texture_view, glyph_sampler) =
            RenderPassBuilder::create_binding_resource_and_sampler(
                device,
                images,
                self.glyph_texture
                    .image_id()
                    .map(ImageOrTexture::Image)
                    .as_ref(),
                empty_texture,
            );
...
impl<'a> RenderPassBuilder<'a> {
    fn new(
        ...
        let viewport_bind_group =
            Self::create_viewport_bind_group(&device, &screen_view, &viewport_bind_group_layout);
        Self {
            device: device.clone(),
            encoder,
    ...
    fn set_viewport(&mut self, viewport: [f32; 2]) {
        ...
        self.viewport_bind_group = Self::create_viewport_bind_group(
            &self.device,
            &self.viewport,
    ...
    fn create_binding_resource_and_sampler(
        ...
        let flags = image
            .and_then(|image_or_texture| match image_or_texture {
                ImageOrTexture::Image(image_id) => {
                    images.get(*image_id).map(|img| img.info.flags())
                }
                _ => None,
            })
        ...
        let binding_resource = image
            .and_then(|image_or_texture| match image_or_texture {
                ImageOrTexture::Image(image_id) => {
                    images.get(*image_id).map(|img| match &img.texture {
                        Texture::Internal(texture) => OwnedBindingResource::TextureView(
                            texture.create_view(&Default::default()),
                        ),
                        Texture::External(texture) => {
                            OwnedBindingResource::ExternalTexture(texture.clone())
                        }
                    })
                }
            ...
            .unwrap_or_else(|| {
                OwnedBindingResource::TextureView(empty_texture.create_view(&Default::default()))
            });
    ...
    fn recreate_render_pass(&mut self, load: wgpu::LoadOp<wgpu::Color>) {
        ...
        let mut rpass = self.encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            ...
            depth_stencil_attachment: stencil_view.as_ref().map(|view| {
...
impl CommandToPipelineAndBindGroupMapper {
    ...
    fn update_renderpass<'a>(
        ...
        render_pipeline.unused_flushes = 0;
...
mod cache_regressions {
    ...
    #[test]
    #[ignore = "Requires an available native GPU; run explicitly in rendering validation"]
    fn clear_and_content_flushes_reuse_pipelines_then_release_idle_entries() {
        let instance = wgpu::Instance::default();
        let adapter =
            wait(instance.request_adapter(&wgpu::RequestAdapterOptions::default())).unwrap();
        let (device, queue) =
            wait(adapter.request_device(&wgpu::DeviceDescriptor::default())).unwrap();
        let target = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("Echo pipeline regression"),
            size: wgpu::Extent3d {
                width: 64,
                height: 64,
                depth_or_array_layers: 1,
        ...
        let mut canvas = crate::Canvas::new(WGPURenderer::new(device, queue.clone())).unwrap();
        canvas.set_size(64, 64, 1.0);
        let mut path = crate::Path::new();
        path.rect(4.0, 4.0, 48.0, 48.0);
        canvas.clear_rect(0, 0, 64, 64, crate::Color::rgba(0, 0, 0, 0));
        queue.submit(canvas.flush_to_output(output.clone()));
        let clear_keys: std::collections::HashSet<_> = canvas
            .renderer
            .pipeline_cache
            .borrow()
            .keys()
            .cloned()
            .collect();
        canvas.fill_path(&path, &crate::Paint::color(crate::Color::rgb(255, 160, 0)));
        queue.submit(canvas.flush_to_output(output.clone()));
        let initial: HashMap<_, _> = canvas
            .renderer
            .pipeline_cache
            .borrow()
            .iter()
            .map(|(key, value)| (key.clone(), value.pipeline.clone()))
            .collect();
        ...
        for _ in 0..32 {
            ...
            canvas.fill_path(&path, &crate::Paint::color(crate::Color::rgb(255, 160, 0)));
            queue.submit(canvas.flush_to_output(output.clone()));
            let cache = canvas.renderer.pipeline_cache.borrow();
            assert_eq!(cache.len(), initial.len());
            for (key, pipeline) in &initial {
                assert_eq!(&cache[key].pipeline, pipeline);
            }
        }
        // Cached layers can render dozens of frames without rebuilding their
        // shadow/content passes. A later resize must reuse those pipelines.
        for _ in 0..32 {
            canvas.clear_rect(0, 0, 64, 64, crate::Color::rgba(0, 0, 0, 0));
            queue.submit(canvas.flush_to_output(output.clone()));
        }
        ...
        let cache = canvas.renderer.pipeline_cache.borrow();
        assert_eq!(cache.len(), clear_keys.len());
        assert!(cache.keys().all(|key| clear_keys.contains(key)));
    }
}
