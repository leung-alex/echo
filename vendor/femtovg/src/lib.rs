...
pub use text::{
    Align, Atlas, Baseline, DrawCommand, FontId, FontMetrics, GlyphDrawCommands, Quad, RenderMode,
    VariationAxisInfo,
};
    ...
    T: Renderer,
{
    ...
    pub fn create_image_from_native_texture(
        ...
        self.images
            .register_native_texture(&mut self.renderer, texture, info)
    }
    ...
    pub fn create_image_from_external_texture(
        ...
        self.images
            .register_external_texture(&mut self.renderer, texture, info)
    }
    ...
    fn stroke_path_internal(
        ...
        if !stroke.line_dash.is_empty() {
            let dashed_path = path.dashed_with_tolerance(
                &stroke.line_dash,
    ...
    fn render_unclipped_image_blit(
        ...
        let mut to_texture_space_transform =
            Transform2D::scaling(1. / params.extent[0], 1. / params.extent[1]);
        ...
        let (s1, t1) = to_texture_space_transform
            .transform_point(target_rect.x + target_rect.w, target_rect.y + target_rect.h);
    ...
    pub fn font_variation_axes(
        &self,
        font_id: FontId,
    ) -> Result<Vec<VariationAxisInfo>, ErrorKind> {
    ...
    pub fn draw_glyph_commands(&mut self, draw_commands: GlyphDrawCommands, paint: &Paint) {
        ...
        for cmd in draw_commands.alpha_glyphs {
            ...
            self.render_triangles(
                ...
                &transform,
                &paint_flavor,
                GlyphTexture::AlphaMask(cmd.image_id),
            );
        }
    ...
    fn draw_glyph_run(
        ...
        if !color_glyphs.is_empty() {
            ...
            draw_commands
                .alpha_glyphs
                .extend(color_commands.alpha_glyphs);
            draw_commands
                .color_glyphs
                .extend(color_commands.color_glyphs);
        }
    ...
    T: SurfacelessRenderer,
{
    ...
    pub fn flush(&mut self) {
        self.renderer.render_surfaceless(
            &mut self.images,
            &self.verts,
            std::mem::take(&mut self.commands),
        );
...
fn fill_text_selects_atlas_or_path_rendering() {
    ...
    for (description, transform, paint_kind, expect) in cases {
        ...
        let paint = match paint_kind {
            ...
            PaintKind::BigSolid => Paint::color(Color::black())
                .with_font(&[font])
        ...
        let used_atlas = commands
            .iter()
            .any(|c| !matches!(c.glyph_texture, GlyphTexture::None));
