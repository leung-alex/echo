...
Only accesskit.rs is changed: choose string versus numeric accessibility value from the control role, not from parsing user text. Queries such as 0013 must preserve leading zeros and retain UIA ValuePattern. Actual sliders, spin buttons and progress indicators still expose numeric values. Two regression tests cover these cases.
