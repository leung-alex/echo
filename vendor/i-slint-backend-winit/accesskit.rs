...
impl NodeCollection {
    ...
    fn build_node_without_children(
        ...
        if let Some(value) = item.accessible_string_property(AccessibleStringProperty::Value) {
            // Text remains text even when it looks numeric: otherwise UIA
            // drops ValuePattern and leading zeroes after entering e.g. 0013.
            set_accessible_value(&mut node, role, value.as_str());
        }
...
include!("echo_accessibility_value.rs");
