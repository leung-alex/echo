...
impl GraphicsBackend for WGPUBackend {
    ...
    fn begin_surface_rendering(
        ...
        if let Some(snapshot_output) = self.snapshot_output.borrow().clone() {
            return Ok(BeginRendering::Acquired(WGPUWindowSurface::Snapshot(
                snapshot_output,
            )));
        }
    ...
    fn with_graphics_api<R>(
        ...
        if let (Some(instance), Some(device), Some(queue)) = (instance, device, queue) {
            Ok(callback(Some(
                i_slint_core::graphics::create_graphics_api_wgpu_29(instance, device, queue),
            )))
        } else {
            Ok(callback(None))
        }
...
impl FemtoVGRenderer<WGPUBackend> {
    pub fn set_surface(
        ...
        let mut surface_config = surface
            ...
            .unwrap();
        // Echo prioritizes input response: keep one outstanding frame. FIFO/vsync
        // and explicit DWM first-frame synchronization remain enabled.
...
impl GraphicsBackend for WgpuTextureBackend {
    ...
    fn begin_surface_rendering(
        ...
        let render_output = self
            ...
            .borrow()
            .clone()
            .ok_or("No texture set for rendering")?;
        Ok(BeginRendering::Acquired(TextureWindowSurface {
            render_output,
        }))
    }
    ...
    fn with_graphics_api<R>(
        ...
        Ok(callback(Some(
            i_slint_core::graphics::create_graphics_api_wgpu_29(
                self.instance.clone(),
                self.device.clone(),
                self.queue.clone(),
            ),
