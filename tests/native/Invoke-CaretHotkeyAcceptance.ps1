...
$wpfProcess=$null;$echoProcess=$null;$targetProcess=$null;$targetRoot=$null;$target=$null;$blocker=$null;$held=$null;$launchCount=0;$success=$false
...
function Start-Target {
    ...
    $targetStart=[Diagnostics.ProcessStartInfo]::new($Fixture);$targetStart.UseShellExecute=$false;$targetStart.CreateNoWindow=$true
    foreach($arg in $args){$targetStart.ArgumentList.Add($arg)}
    $script:targetProcess=[Diagnostics.Process]::Start($targetStart)
...
function Record-Owned($Process,[string]$Title) {
    ...
    do {
        ...
        Start-Sleep -Milliseconds 20
    } while(!$Process.HasExited -and $clock.ElapsedMilliseconds -lt 5000)
    if(!$executablePath){throw "Could not establish owned process executable identity"}
...
function Assert-Binding([string]$Chord,[bool]$Available) {
    # Deliberate Quit is part of this test: do not use Wait-Until's crash guard.
    $clock=[Diagnostics.Stopwatch]::StartNew()
    do {
        if((F 'probe' @($Chord)).available -eq $Available){return}
        Start-Sleep -Milliseconds 20
    } while($clock.ElapsedMilliseconds -lt 5000)
    throw "Shortcut $Chord availability did not become $Available"
}
...
function Assert-Anchored([string]$Name) {
    ...
    if($gap -gt 40*($lastTargetGeometry.dpi/96.0)){throw "Card is not near the caret: vertical gap=$gap"}
    $proof=[ordered]@{target=$lastTargetGeometry;card=$card;vertical_gap_pixels=$gap;source='independent Win32 caret and UIA card bounds'}
...
try {
    ...
    if($Scope -eq 'feature') {
        . (Join-Path $Root 'tests/native/CaretHotkeyScenarios.ps1')
    }
    ...
    if($wpfProcess -and !$wpfProcess.HasExited){[IO.File]::WriteAllText((Join-Path $EvidenceRoot 'wpf.stop'),'stop');$null=$wpfProcess.WaitForExit(5000)}
