...
export function getSearchMatchIndices(text: string, query: string): number[] {
  ...
  for (const term of query.trim().toLocaleLowerCase().split(/\s+/).filter(Boolean)) {
    ...
    for (let start = 0; start <= folded.length - needle.length; start += 1) {
      if (needle.every((character, offset) => folded[start + offset] === character)) {
        for (let offset = 0; offset < needle.length; offset += 1) matched.add(start + offset);
      }
    }
  }
...
export function SearchMatchText({ text, indices }: { text: string; indices: readonly number[] }): ReactNode {
  const characters = graphemes(text);
  const matched = new Set(indices.filter((index) => index >= 0 && index < characters.length));
  ...
  return parts.map((part, index) =>
    part.matched ? <mark className="echo-search-match" key={index}>{part.text}</mark> : part.text,
  );
}
