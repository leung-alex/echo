...
} from "../quick-insert/api/quick-insert-client";
import {
  clearHistory,
  getSettings,
...
export function SettingsPage({
  ...
  const reload = async () => {
    try {
      const [nextSettings, nextSnippets] = await Promise.all([
        getSettings(),
        client.listSnippets(""),
      ]);
  ...
  const saveSnippet = async () => {
    if (!form.name.trim() || !form.content.trim()) {
      report("Snippet name and content are required", "error");
      return;
    }
    try {
      await client.saveSnippet(
        editing,
        form.name.trim(),
        form.content,
        form.group_name.trim() || null,
      );
      setForm({ name: "", content: "", group_name: "" });
      setEditing(null);
      setFormOpen(false);
      setSnippets(await client.listSnippets(""));
      report("Snippet saved", "success");
    } catch (error) {
      report(error instanceof Error ? error.message : String(error), "error");
    }
  };
  const removeSnippet = async (snippet: EchoSnippet) => {
    ...
    try {
      await client.deleteSnippet(snippet.id);
      setSnippets(await client.listSnippets(""));
      report("Snippet deleted", "success");
    } catch (error) {
      report(error instanceof Error ? error.message : String(error), "error");
    }
  };
  const editSnippet = (snippet: EchoSnippet) => {
    ...
    <main className="clipboard-settings-window">
      <header className="settings-header">
        ...
        <div>
          ...
          <p>Recording, privacy, and reusable snippets.</p>
        </div>
      </header>
      ...
      <section className="settings-snippets" aria-labelledby="snippets-heading">
        <div className="settings-section-heading">
          <div>
            <h2 id="snippets-heading">Snippets</h2>
            <p>Reusable text available in Quick Insert.</p>
          </div>
          <button
            type="button"
            onClick={() => {
          ...
          >
            <Plus size={15} aria-hidden="true" /> New snippet
          </button>
        </div>
        {formOpen ? (
          <div className="snippet-form">
            <h3>{editing === null ? "New snippet" : "Edit snippet"}</h3>
            <label>
              Snippet name
              <input
        ...
        <div className="settings-snippet-list">
          {snippets.length === 0 ? (
            <p className="settings-empty">No snippets yet.</p>
          ) : (
            snippets.map((snippet) => (
              <article className="settings-snippet-row" key={snippet.id}>
                <div>
                  <strong>{snippet.name}</strong>
                  <span>{snippet.group_name || "Ungrouped"}</span>
                  <p>{snippet.content}</p>
                </div>
                <div>
                  <button
