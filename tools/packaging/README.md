...
License text is copied verbatim from resolved packages and deduplicated by SHA256. resolved-licenses.json maps packages to original text. The inventory follows the default Windows desktop runtime/build dependency tree; inventory inclusion does not mean a DLL is shipped. Some published crates include only license metadata; their declared source/license remain in the notices for review. No font files are included.
